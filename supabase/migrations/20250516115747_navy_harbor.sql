...
CREATE POLICY "Enable read access for device working hours"
ON device_working_hours
FOR SELECT
TO authenticated
USING (true);

CREATE POLICY "Enable admin access for device working hours"
ON device_working_hours
FOR ALL
TO authenticated
...
CREATE POLICY "Enable read access for device exceptions"
ON device_exceptions
FOR SELECT
TO authenticated
USING (true);

CREATE POLICY "Enable admin access for device exceptions"
ON device_exceptions
FOR ALL
TO authenticated
USING (auth.jwt() ->> 'role' = 'admin')
